...
podman run --name ocp4-auto-install --tty --workdir /opt/ocp4-auto-install --privileged --volume=/sys/fs/cgroup:/sys/fs/cgroup:ro --volume=`pwd`:/opt/ocp4-auto-install ocp4-auto-install:latest ansible-playbook -i ,localhost deploy.yml --vault-password-file .vault-password-file
